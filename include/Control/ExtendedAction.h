...
namespace CartWheel {
    ...
    enum UnitType {
        ...
        TIME_UNIT, // seconds
        OTHER_UNIT
    }; // placeholder (how to handle oddballs?)
    ...
    public:
        ...
        virtual void setActor(string s) {
            actorName = s;
        };
        ...
        virtual void execute(CartWheel3D *cw, std::vector<PosState*> *posStates);
        virtual void executeStep(CartWheel3D *cw, double step);

        //virtual void executeSetup(CartWheel3D * cw) = 0;

        virtual int executeSetup(CartWheel3D * cw) = 0;
