...
PickUp::PickUp(CartWheel3D* cw, std::string humanName, PickUp_Params* params)
    ...
    bPickUpFinished = false;
}

void PickUp::onInit() {
    printf("starting to pickup\n");
}
...
//    human->setVelocity("rHand", Vector3d(0,1,0));
    ...
    nSteps++;
    if (nSteps == SimGlobals::steps_per_second/2 && !bPickUpFinished) {
        nSteps = 0;        
...
////        cw->getObjectByName("boxHandle2 boxH")->setCMVelocity(Vector3d(0,0,0));
        cw->getObjectByName(sTargetObj)->setCMVelocity(Vector3d(0,0,0));
    }
}

void PickUp::onFinish() {
    printf("Ending PickUp Time: %f\n", time);
    resetHumanAngles();
}

double PickUp::getBendOverK(Point3d pObj) {
...
double PickUp::getDistXZ(Point3d p1, Point3d p2) {
    double d = sqrt(pow(p1.x - p2.x, 2) + pow(p1.z - p2.z, 2));
    return d;
}

double PickUp::getDistXYZ(Point3d p1, Point3d p2) {
    double d = sqrt(pow(p1.x - p2.x, 2) + pow(p1.y - p2.y, 2) + pow(p1.z - p2.z, 2));
    return d;
}

void PickUp::resetHumanAngles() {
