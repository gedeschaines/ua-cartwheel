...
void IKVMCController::computeIKSwingLegTargets(double dt) {
    ...
    //printf("\nIKVMC: %d, %d, %d\n", swingHipIndex, swingKneeIndex, swingAnkleIndex);
    Vector3d parentAxis(character->joints[swingHipIndex]->getChildJointPosition(), character->joints[swingKneeIndex]->getParentJointPosition());
    Vector3d childAxis(character->joints[swingKneeIndex]->getChildJointPosition(), character->joints[swingAnkleIndex]->getParentJointPosition());
    ...
    for (int i = character->joints.size() - 1; i >= 0; i--) {
        if (i != stanceHipIndex && i != stanceKneeIndex)
            if (character->joints[i]->getParent() != root)
                torques[character->joints[i]->getParent()->getParentJoint()->getID()] += torques[i];
    }
}
...
void IKVMCController::computeGravityCompensationTorques() {
    vmc->resetTorques();
    for (unsigned int i = 0; i < character->joints.size(); i++) {
        if (i != (size_t) stanceHipIndex && i != (size_t) stanceKneeIndex && i != (size_t) stanceAnkleIndex)
            vmc->computeJointTorquesEquivalentToForce(character->joints[i], Point3d(), Vector3d(0, character->joints[i]->getChild()->getMass()*9.8, 0), NULL);
    }
...
void IKVMCController::preprocessAnkleVTorque(int ankleJointIndex, DynamicArray<ContactPoint> *cfs, Vector3d *ankleVTorque) {
    ...
    ArticulatedRigidBody* foot = character->joints[ankleJointIndex]->getChild();
...
void IKVMCController::COMJT(DynamicArray<ContactPoint> *cfs) {
    ...
    double m = 0;
    ArticulatedRigidBody* tibia = character->joints[stanceAnkleIndex]->getParent();
    ArticulatedRigidBody* femur = character->joints[stanceKneeIndex]->getParent();
    ArticulatedRigidBody* pelvis = character->joints[stanceHipIndex]->getParent();
    ArticulatedRigidBody* lBack = character->joints[lBackIndex]->getChild();
    ArticulatedRigidBody* mBack = character->joints[mBackIndex]->getChild();

    Point3d anklePos = character->joints[stanceAnkleIndex]->getChild()->getWorldCoordinates(character->joints[stanceAnkleIndex]->getChildJointPosition());
    Point3d kneePos = character->joints[stanceKneeIndex]->getChild()->getWorldCoordinates(character->joints[stanceKneeIndex]->getChildJointPosition());
    Point3d hipPos = character->joints[stanceHipIndex]->getChild()->getWorldCoordinates(character->joints[stanceHipIndex]->getChildJointPosition());
    Point3d lbackPos = character->joints[lBackIndex]->getChild()->getWorldCoordinates(character->joints[lBackIndex]->getChildJointPosition());
    Point3d mbackPos = character->joints[mBackIndex]->getChild()->getWorldCoordinates(character->joints[mBackIndex]->getChildJointPosition());
    ...
    Vector3d f1 = Vector3d(anklePos, tibia->getCMPosition()) * tibia->getMass() +
            ...
            Vector3d(anklePos, lBack->getCMPosition()) * lBack->getMass() +
            Vector3d(anklePos, mBack->getCMPosition()) * mBack->getMass();
    f1 /= m;

    Vector3d f2 = Vector3d(kneePos, femur->getCMPosition()) * femur->getMass() +
            Vector3d(kneePos, pelvis->getCMPosition()) * pelvis->getMass() +
            Vector3d(kneePos, lBack->getCMPosition()) * lBack->getMass() +
            Vector3d(kneePos, mBack->getCMPosition()) * mBack->getMass();
    f2 /= m;

    Vector3d f3 = Vector3d(hipPos, pelvis->getCMPosition()) * pelvis->getMass() +
            Vector3d(hipPos, lBack->getCMPosition()) * lBack->getMass() +
            Vector3d(hipPos, mBack->getCMPosition()) * mBack->getMass();
    f3 /= m;

    Vector3d f4 = Vector3d(lbackPos, lBack->getCMPosition()) * lBack->getMass() +
            Vector3d(lbackPos, mBack->getCMPosition()) * mBack->getMass();
    f4 /= m;

    Vector3d f5 = Vector3d(mbackPos, mBack->getCMPosition()) * mBack->getMass();
    f5 /= m;
...
void IKVMCController::performPreTasks(double dt, DynamicArray<ContactPoint> *cfs) {
    //std::cout << "IKVMCController::performPreTasks" << std::endl;
    if (behaviour != NULL)
