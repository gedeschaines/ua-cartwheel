...
void SimBiController::setFSMStateTo(int index) {
    if (index < 0)
        FSMStateIndex = -1;
    else if ((unsigned int) index >= states.size())
...
bool SimBiController::haveRelationBetween(RigidBody* rb, RigidBody* whichBody) {
    ...
    if (rb == whichBody)
        return true;
    for (unsigned int j = 0; j < (size_t) ((ArticulatedRigidBody*) whichBody)->getChildJointCount(); j++)
        if (((ArticulatedRigidBody*) whichBody)->getChildJoint(j)->getChild() == rb)
            return true;
    return false;
}
...
bool SimBiController::isFoot(RigidBody* rb) {
    ...
    for (unsigned int j = 0; j < (size_t) ((ArticulatedRigidBody*) lFoot)->getChildJointCount(); j++)
        if (((ArticulatedRigidBody*) lFoot)->getChildJoint(j)->getChild() == rb)
            return true;
    for (unsigned int j = 0; j < (size_t) ((ArticulatedRigidBody*) rFoot)->getChildJointCount(); j++)
        if (((ArticulatedRigidBody*) rFoot)->getChildJoint(j)->getChild() == rb)
            return true;
...
bool SimBiController::isSwingFoot(RigidBody* rb) {
    ...
    if (rb == swingFoot)
        return true;
    //and against the toes
    for (unsigned int j = 0; j < (size_t) ((ArticulatedRigidBody*) swingFoot)->getChildJointCount(); j++)
        if (((ArticulatedRigidBody*) swingFoot)->getChildJoint(j)->getChild() == rb)
            return true;
    return false;
}
...
bool SimBiController::isStanceFoot(RigidBody* rb) {
    ...
    if (rb == stanceFoot)
        return true;
    //and against the toes
    for (unsigned int j = 0; j < (size_t) ((ArticulatedRigidBody*) stanceFoot)->getChildJointCount(); j++)
        if (((ArticulatedRigidBody*) stanceFoot)->getChildJoint(j)->getChild() == rb)
            return true;
    return false;
}
...
Vector3d SimBiController::getForceOnFoot(RigidBody* foot, DynamicArray<ContactPoint> *cfs) {
    ...
    for (unsigned int i = 0; i < (size_t) ((ArticulatedRigidBody*) foot)->getChildJointCount(); i++) {
        fNet += getForceOn(((ArticulatedRigidBody*) foot)->getChildJoint(i)->getChild(), cfs);
    }
    return fNet;
}
...
int SimBiController::advanceInTime(double dt, DynamicArray<ContactPoint> *cfs) {
    ...
    if (FSMStateIndex >= (int) states.size()) {
        printf("Warning: no FSM state was selected in the controller!\n");
        return -1;
    }
    ...
    for (int i = 0; i < jointCount; i++) {
        if (controlParams[i].relToFrame) {
            ...
            while (parent != root) {
                ...
                parent = j->getParent();
                temp = debugRS.getJointRelativeOrientation(character->getJointIndex(j->getName())) * temp;
            }
 ...
 */
RigidBody* SimBiController::getRBBySymbolicName(const char* sName) {
    RigidBody* result;
...
void SimBiController::computePDTorques(DynamicArray<ContactPoint> *cfs) {
    ...
    //std::cout << "SimBiController::computePDTorques" << std::endl;
...
void SimBiController::computeTorques(DynamicArray<ContactPoint> *cfs) {
    //printf("Running SimBiController::computeTorques...\n");
...
void SimBiController::computeHipTorques(const Quaternion& qRootD, double stanceHipToSwingHipRatio, Vector3d ffRootTorque) {
    ...
    if (stanceHipDamping > 0) {
        Vector3d wRel = root->getAngularVelocity() - character->joints[stanceHipIndex]->getChild()->getAngularVelocity();
...
void SimBiController::resolveJoints(SimBiConState* state) {
    char tmpName[100];
    for (unsigned int i = 0; i < state->sExternalForces.size(); i++) {
    ...
    for (unsigned int i = 0; i < state->sTraj.size(); i++) {
 ...
 */
void SimBiController::writeToFile(const char* fileName, const char* stateFileName) {
    FILE* f;
 ...
 */
void SimBiController::parseGainLine(const char* line) {
...
void SimBiController::loadFromFile(const char* fName) {
    ...
    while (!feof(f)) {
        ...
        //printf("Reading cline: %s. LineType=%d\n", line, lineType);
        switch (lineType) {
 ...
 */
SimBiConState* SimBiController::getState(unsigned int idx) {
