...
void ODEWorld::setODEStateFromRB(int i) {
    if (i < 0 || (unsigned int) i >= odeToRbs.size())
        return;

    //if it is a locked object, we update its CDPS
    if (odeToRbs[i].rb->isLocked() == true) {

        for (unsigned int j = 0; j < odeToRbs[i].collisionVolumes.size(); j++) {
...
void ODEWorld::setupODEBallAndSocketJoint(BallInSocketJoint* basj) {
    ...
    //printf("Inside setupODEBallAndSocketJoint 3\n");
    ...
    //printf("Inside setupODEBallAndSocketJoint lines further\n");
...
void ODEWorld::createODECollisionPrimitives(RigidBody* body, int index) {
    ...
    for (unsigned int j = 0; j < body->cdps.size(); j++) {
...
void ODEWorld::loadRBsFromFile(const char* fName, const char* sPath, const char* afName) {
    ...
    for (unsigned int i = index_afs; i < AFs.size(); i++) {
...
void ODEWorld::processCollisions(dGeomID o1, dGeomID o2) {
    ...
    for (int i = 0; i < num_contacts; i++) {
        ...
        if (jointFeedbackCount >= MAX_CONTACT_FEEDBACK)
            ...
            if (contactPoints.size() != (size_t) jointFeedbackCount) {
                printf("Warning: Contact forces need to be cleared after each simulation, otherwise the results are not predictable.\n");
            }
...
void ODEWorld::advanceInTime(double deltaT) {
    ...
    for (unsigned int j = 0; j < jts.size(); j++) {
