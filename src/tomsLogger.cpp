...
int main(int argc, char** argv)
{
   ...
   for (double curSpeed = 0.0; curSpeed <= 3.0; curSpeed += 0.25) {
      ...
      for (int i = 0; i < trajectory.size(); ++i)
      {
         ...
         //{
         pos_j = pos_state->getPosition(j);
         pos_t = trajectory[0]->getPosition(0);
         del_x = pos_j.x - pos_t.x;
         del_z = pos_j.z - pos_t.z;
         cout << sqrt( (del_x * del_x) +  (del_z * del_z)) << ",";
         //}
      }
      cout << endl << endl;
     //delete actions[0];
